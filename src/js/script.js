...
const searchIcon = navigation.querySelector('.navigation__nav--list__item--search__icon');
...
const closeRegister = () => {
    ...
    if (element.contains('register') || element.contains('register__window--close')) {
...
const removeSearchBackground = () => {
    ...
    if (element.classList.contains('search') || element.closest('a').classList.contains('search__form--btn')) {
...
navigationIcon.addEventListener('click', event => {
    const element = event.target.firstElementChild === null ? event.target.closest('span') : event.target.firstElementChild;
