...
const carouselNavigators = document.querySelectorAll('.carousel__navigator div');
...
carouselNavigators.forEach(element => {
    element.addEventListener('click', event => {
        const direction = event.target.className.includes('left') ? 'left' : 'right';
        for (el of carouselInner) {
            if (el.classList.contains('carousel__inner--slide__active')) {
                const slide = el;
                const move = direction === 'left' ? slide.previousElementSibling === null ? slide.parentElement.lastElementChild.classList : slide.previousElementSibling.classList : slide.nextElementSibling === null ? slide.parentElement.firstElementChild.classList : slide.nextElementSibling.classList;
...
const createProducts = (number, catagory, data) => {
    const targetElement = catagory === 'featured' ? featuredProducts : catagory === 'bestselling' ? bestSellingProducts : 'talha';
...
const fetchProductData = (data, catagory) => {
    ...
    const requiredData = catagory === 'featured' ?
        data.reverse() :
        data
        .map((element) => {
            return element.sold;
